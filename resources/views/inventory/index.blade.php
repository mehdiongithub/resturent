        ...
        <div class="col-md-12">
          <div class="card">
            <div class="card-header">
              <h4 class="card-title">Inventories Data</h4>
            </div>
            <div class="card-body">
                    ...
                    @foreach ($data as $inventory)
                        <tr>
                            ...
                            <td>
                                ...
                                <a href="{{ route('inventories.edit', $inventory->id) }}" class="action-icon">
                                    <i class="fas fa-edit"></i>
                                </a>
                
                                <!-- Delete Form with Icon and Text -->
                                <form id="deleteForm{{ $inventory->id }}" action="{{ route('inventories.destroy', $inventory->id) }}" method="POST" style="display:inline;">
                                    @csrf
                                    @method('DELETE')
                                    <a href="javascript:void(0);" class="action-icon" data-bs-toggle="modal" data-bs-target="#exampleModal"
                                        data-title="inventory" data-form-id="deleteForm{{ $inventory->id }}">
...
<script>
    $(document).ready(function () {
      $("#basic-datatables").DataTable({
        "columnDefs": [
          {
            "targets": 7, // Disable sorting for the 'Action' column (index 4)
            "orderable": false
          }
        ]
      });
    ...
    var IndexUrl = "{{ route('inventories.index') }}";
